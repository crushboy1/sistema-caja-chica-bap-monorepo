  ...
  <div class="min-h-screen bg-verde-bap-extralight p-6">
    <div class="max-w-7xl mx-auto space-y-8">
      ...
      <div class="bg-white rounded-xl shadow-lg p-8 mb-8 animate-fade-in-down">
        <div class="text-center">
          ...
          <p class="text-gray-600 max-w-4xl mx-auto text-base leading-relaxed">
            ¡Bienvenido! En este espacio podrás registrar toda la información relacionada con los montos utilizados, adjuntar los comprobantes o justificantes correspondientes y detallar cada gasto realizado. Este proceso garantiza la transparencia y el adecuado control de los fondos asignados.
          </p>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mt-12">
        ...
        <!-- Card 1: Declaración de Gasto (Estilo Verde BAP) -->
        <div v-if="showDeclaracionCard" @click="handleCardClick('declaracion')" 
          ...
          :class="getCardClasses('declaracion')" 
          style="animation-delay: 0.1s"
          tabindex="0" @keydown.enter="handleCardClick('declaracion')"
          ...
          <div class="relative z-10 p-4 text-center text-white h-full flex flex-col justify-center">
            <div class="mb-2 flex justify-center">
              <div class="w-12 h-12 bg-white/25 backdrop-blur-sm rounded-2xl flex items-center justify-center group-hover:bg-white/35 transition-all duration-500 group-hover:scale-110 group-hover:rotate-12 shadow-medium">
                <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
              </div>
            </div>
            <h3 class="text-base md:text-lg font-bold mb-2 group-hover:tracking-wide transition-all duration-300">DECLARACIÓN DE GASTO</h3>
            <p class="text-xs text-white/90 group-hover:text-white transition-all duration-300">Registrar un nuevo gasto</p>
          </div>
        </div>

        <!-- Card 2: Aprobaciones (Estilo Amarillo BAP) -->
        <div v-if="showAprobacionesCard" @click="handleCardClick('aprobaciones')"
          class="group relative overflow-hidden rounded-3xl cursor-pointer
          ...
          :class="getCardClasses('aprobaciones')" 
          style="animation-delay: 0.2s"
          tabindex="0" @keydown.enter="handleCardClick('aprobaciones')"
          ...
          <div class="relative z-10 p-4 text-center text-gray-800 h-full flex flex-col justify-center">
            <div class="mb-2 flex justify-center">
                <div class="w-12 h-12 bg-white/40 backdrop-blur-sm rounded-2xl flex items-center justify-center group-hover:bg-white/50 transition-all duration-500 group-hover:scale-110 group-hover:-rotate-12 shadow-medium">
                    <svg class="w-6 h-6 text-amarillo-bap-dark" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                </div>
            </div>
            <h3 class="text-base md:text-lg font-bold mb-2 group-hover:text-gray-900 transition-all duration-300">APROBACIONES DE GASTOS</h3>
            <p class="text-xs text-gray-700 group-hover:text-gray-800 transition-all duration-300">Validar gastos de tu equipo</p>
          </div>
        </div>

        <!-- Card 3: Base de Datos y Auditoría (Estilo Rojo BAP) -->
        <div v-if="showAuditoriaCard" @click="handleCardClick('auditoria')"
            class="group relative overflow-hidden rounded-3xl cursor-pointer
            ...
            :class="getCardClasses('auditoria')" 
            ...
            <div class="relative z-10 p-4 text-center text-white h-full flex flex-col justify-center">
                <div class="mb-2 flex justify-center">
                    <div class="w-12 h-12 bg-white/10 backdrop-blur-sm rounded-2xl flex items-center justify-center group-hover:bg-white/20 transition-all duration-500 group-hover:scale-110 group-hover:animate-bounce-gentle shadow-medium">
                        <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 7v10c0 2.21 3.582 4 8 4s8-1.79 8-4V7M4 7c0-2.21 3.582-4 8-4s8 1.79 8 4M4 7s0 4 8 4s8-4 8-4"></path><line x1="4" y1="12" x2="4" y2="12"></line><line x1="4" y1="17" x2="4" y2="17"></line><line x1="20" y1="12" x2="20" y2="12"></line></svg>
                    </div>
                </div>
                <h3 class="text-base md:text-lg font-bold mb-2 group-hover:tracking-wide transition-all duration-300">AUDITORÍA Y REPORTES</h3>
                <p class="text-xs text-white/90 group-hover:text-white transition-all duration-300">Revisar gastos y exportar a SAP</p>
            </div>
        </div>
      ...
      <Transition enter-active-class="transition-all duration-500 ease-out"
        ...
        <div v-if="activeSection" class="mt-12">
          <div class="bg-white/80 backdrop-blur-xl border border-white/20 rounded-3xl p-8 shadow-strong">
            <!-- Aquí se cargarán los componentes de cada sub-módulo -->
            <DeclaracionGasto v-if="activeSection === 'declaracion'" @close="handleClose" class="animate-fade-in-up" />
            <AprobacionGastos v-if="activeSection === 'aprobaciones'" @close="handleClose" class="animate-fade-in-up" />
            <AuditoriaGastos v-if="activeSection === 'auditoria'" @close="handleClose" class="animate-fade-in-up" />
          </div>
        </div>
      </Transition>
      ...
      <Transition enter-active-class="transition-all duration-600 ease-out delay-500"
        ...
        <div v-if="!activeSection" class="relative overflow-hidden rounded-2xl bg-verde-bap-extralight border-l-4 border-verde-bap p-6 shadow-soft">
          <div class="relative flex items-start space-x-4">
            ...
            <div class="flex-1">
              <p class="text-gray-700 text-base leading-relaxed">
                <strong class="font-semibold">Nota importante:</strong> Los gastos deben rendirse hasta el día 30 de cada mes. Todo gasto efectuado debe contar con la documentación correspondiente que lo sustente (comprobante de pago o declaración jurada).
              </p>
            </div>
          </div>
        </div>
...
// Importar componentes del módulo de declaraciones (cuando existan)
